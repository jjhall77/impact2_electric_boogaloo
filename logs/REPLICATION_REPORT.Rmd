---
title: "Replication Report: MacDonald, Fagan & Geller (2016)"
subtitle: |
  "The Effects of Local Police Surges on Crime and Arrests in New York City"
  \newline PLOS ONE, 2016. DOI: 10.1371/journal.pone.0157223
...
2. **Blocks span multiple precincts.** 1,588 of 6,274 unique fids appear
   in more than one precinct. The observation unit is fid $\times$ pct, not fid.
...
|:------|:--------|:--------------|:------|
| M1 | $Y \sim \text{treatment}$ | pct $\times$ month-year | Basic treatment effect |
| M2 | $Y \sim \text{treatment} + \text{treatmentn}$ | pct $\times$ month-year | Neighbor spillover |
| M3 | $Y \sim \text{event}_{-2} + \text{event}_{-1} + \text{event}_{+1} + \text{event}_{+2}$ | pct $\times$ month-year | Event study ($\pm$2 months) |
| M4 | $Y \sim \text{treatment} + \text{cs\_pc} + \text{cs\_npc} + \text{trt} \times \text{pc} + \text{trt} \times \text{npc}$ | pct $\times$ month-year | PC vs NPC stop interactions |
| M5 | $Y \sim \text{treatment} + bs_1 + bs_2 + bs_3$ | fid | Cubic polynomial trend |
...
# Phase 1 Results: Exact Replication
...
year $\times$ precinct $\times$ month groups, re-estimating Model 1 each time.
...
tend to be slightly larger, consistent with known differences between
fixest's clustered sandwich estimator and Stata's implementation.


# Phase 3 Results: Staggered DD Extensions
...
(6,274 fids $\times$ 36 quarters = 225,864 obs). Treatment cohorts are defined
...
3. **Callaway & Sant'Anna ($-$0.857)** gives a much larger estimate but
   with wide confidence intervals. C&S dropped the earliest cohort
...
The Sun & Abraham event study shows pre-treatment coefficients that are
noisy but centered around zero, consistent with parallel trends.
...
The Callaway & Sant'Anna event study shows a similar pattern with wider
confidence bands.
